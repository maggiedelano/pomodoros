...
%   OUTPUT_ARRAY = SLIDINGVAR(INPUT_ARRAY, N)
%
%  The function 'slidingvar' implements a one-dimensional filtering, applying a sliding window to a sequence. Such filtering replaces the center value in
...
%  vector INPUT_ARRAY, the variance is computed among the available points. Indicating with nx the length of the the input sequence, we note that for values
%  of N larger or equal to 2*(nx - 1), each value of the output data array are identical and equal to var(in).
%
